...
class _ReportLostItemScreenState extends State<ReportLostItemScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      print('📝 Starting to save lost item for user: ${user.uid}');

      // Get user details
      final userModel = await _userService.getUserById(user.uid);
      if (userModel == null) {
        print('⚠️ User details not found in Firestore. User ID: ${user.uid}');
        throw Exception('User details not found. Please complete your profile first.');
      }

      print('✅ User details found: ${userModel.fullName}, ${userModel.phoneNumber}');
      ...
      print('📦 Item created: ${item.title}, ID: ${item.itemId}');

      // Save to Firestore
      await _itemService.saveLostItem(item);

      print('✅ Lost item saved successfully!');
